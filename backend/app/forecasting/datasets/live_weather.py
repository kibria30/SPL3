...
def load_dataframe(latitude: float = DEFAULT_LATITUDE, longitude: float = DEFAULT_LONGITUDE,
    ...
    return df[HOURLY_VARS].interpolate(method="linear", limit_direction="both")
...
def load(seq_len: int = 96, pred_len: int = 96, latitude: float = DEFAULT_LATITUDE,
    ...
    df = load_dataframe(latitude=latitude, longitude=longitude, start_date=start_date, end_date=end_date)
